            ...
            <div class="row">
                <div class="col-lg-3">
                    <div class="list-group">
                        ...
                        <a href="{{Session::get('base_url')}}/biaya_overhead_tetap/{{$produksi_id}}" class="list-group-item list-group-item-action active" aria-current="true">
...
@push('script')
    ...
    <script>
         function loadtable() {
            ...
            setTimeout(() => {
                $('#table').load('../table_biaya_overhead_tetap/'+id, function() {
                    $('#load_table').hide();
                });
            }, 500);
        }
        ...
        function editdata(value) {
            ...
            $('#edittotal').val('');
            setTimeout(() => {
                $.get('{{ url('') }}/api/biaya_overhead_tetap/show/' + value, function(data,
                    status) {
                    if (data.error == false) {
                        $('#editid').val(value);
        ...
        $(document).ready(function() {
            ...
            $('#forminput').on('submit', function(e) {
                ...
                $.ajax({
                    type: "POST",
                    url: "{{ url('') }}/api/biaya_overhead_tetap/create/"+id,
                    dataType: "json",
                    data: datas
                }).done(function(data) {
                    if (data.error == false) {
            ...
            $('#formupdate').on('submit', function(e) {
                ...
                $.ajax({
                    type: "POST",
                    url: "{{ url('') }}/api/biaya_overhead_tetap/update/"+id,
                    dataType: "json",
                    data: datas
                }).done(function(data) {
                    if (data.error == false) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        type: "DELETE",
                        dataType: "json",
                        url: "{{ url('') }}/api/biaya_overhead_tetap/delete/" + value
                    }).done(function(data) {
                        if (data.error == false) {
                            Swal.fire({
                                icon: 'success',
