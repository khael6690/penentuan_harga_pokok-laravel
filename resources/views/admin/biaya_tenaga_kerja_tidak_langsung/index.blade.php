            ...
            <div class="row">
                <div class="col-lg-3">
                    <div class="list-group">
                        ...
                        <a href="{{Session::get('base_url')}}/biaya_tenaga_kerja_tidak_langsung/{{$produksi_id}}" class="list-group-item list-group-item-action active" aria-current="true">
                                                    ...
                                                    <div class="mb-3">
                                                        ...
                                                        <div id="refreshselect">
                                                            ...
                                                            data-placeholder="Pilih Tenaga Kerja">
                                                                ...
                                                                @inject('BiayaTKTidakLangsung', 'App\Models\BiayaTKTidakLangsung')
                                                                @foreach ($tenaga_kerja as $item)
                                                                @php
                                                                    $biaya_tenaga_kerja = $BiayaTKTidakLangsung::where('tenaga_kerja_id',$item->id)->where('produksi_id',$produksi_id)->first();
                                                                @endphp
...
@push('script')
    ...
    <script>
         function loadtable() {
            ...
            setTimeout(() => {
                $('#table').load('../table_biaya_tenaga_kerja_tidak_langsung/'+id, function() {
                    $('#load_table').hide();
                });
            }, 500);
        }
        ...
        function editdata(value) {
            ...
            $('#editbagian').val('');
            setTimeout(() => {
                $.get('{{ url('') }}/api/biaya_tenaga_kerja_tidak_langsung/show/' + value, function(data,
                    status) {
                    if (data.error == false) {
                        $('#editid').val(value);
        ...
        $(document).ready(function() {
            ...
            $('#forminput').on('submit', function(e) {
                ...
                $.ajax({
                    type: "POST",
                    url: "{{ url('') }}/api/biaya_tenaga_kerja_tidak_langsung/create/"+id,
                    dataType: "json",
                    data: datas
                }).done(function(data) {
                    if (data.error == false) {
            ...
            $('#formupdate').on('submit', function(e) {
                ...
                $.ajax({
                    type: "POST",
                    url: "{{ url('') }}/api/biaya_tenaga_kerja_tidak_langsung/update/"+id,
                    dataType: "json",
                    data: datas
                }).done(function(data) {
                    if (data.error == false) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        type: "DELETE",
                        dataType: "json",
                        url: "{{ url('') }}/api/biaya_tenaga_kerja_tidak_langsung/delete/" + value
                    }).done(function(data) {
                        if (data.error == false) {
                            Swal.fire({
                                icon: 'success',
