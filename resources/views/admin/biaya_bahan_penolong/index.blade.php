            ...
            <div class="row">
                <div class="col-lg-3">
                    <div class="list-group">
                        ...
                        <a href="{{Session::get('base_url')}}/biaya_bahan_penolong/{{$produksi_id}}" class="list-group-item list-group-item-action active" aria-current="true">
                                                ...
                                                <form id="forminput">
                                                    ...
                                                    <div class="mb-3">
                                                        ...
                                                        <div id="refreshselect">
                                                            <select class="form-select" id="bahan_id" name="bahan_id"
                                                                data-placeholder="Pilih Bahan">
                                                                <option value="" disabled selected>Pilih Bahan</option>
                                                                @inject('BiayaBahanPenolong', 'App\Models\BiayaBahanPenolong')
                                                                @foreach ($bahan_penolong as $item)
                                                                @php
                                                                    $biaya_bahan_penolong = $BiayaBahanPenolong::where('bahan_id',$item->id)->where('produksi_id',$produksi_id)->first();
                                                                @endphp
                                                                    <option value="{{ $item->id }} {{!is_null($biaya_bahan_penolong) ? 'disabled' : ''}}">{{ $item->nama_bahan }} </option>
                                                                @endforeach
                                                            </select>
                                                        </div>
                                                    </div>
                                                ...
                                                <form id="formupdate">
                                                    ...
                                                    <div class="mb-3">
                                                        ...
                                                        <div id="refreshselect2">
                                                            <select class="form-select" id="editbahan_id" name="bahan_id"
                                                                data-placeholder="Pilih Bahan">
                                                                <option value="" disabled selected>Pilih Bahan</option>
                                                                @inject('BiayaBahanPenolong', 'App\Models\BiayaBahanPenolong')
                                                                @foreach ($bahan_penolong as $item)
...
@push('script')
    ...
    <script>
         function loadtable() {
            ...
            setTimeout(() => {
                $('#table').load('../table_biaya_bahan_penolong/'+id, function() {
                    $('#load_table').hide();
                });
            }, 500);
        }
        ...
        function editdata(value) {
            ...
            $('#editqty').val('');
            setTimeout(() => {
                $.get('{{ url('') }}/api/biaya_bahan_penolong/show/' + value, function(data,
                    status) {
                    if (data.error == false) {
                        $('#editid').val(value);
                        $('#editbahan_id').val(data.result.bahan_id);
                        $('#editqty').val(data.result.qty);
                        $("#editbahan_id option:selected").removeAttr('disabled').siblings().attr('disabled','disabled');
                    }
                });
            }, 100);
        }
        ...
        $(document).ready(function() {
            ...
            $('#forminput').on('submit', function(e) {
                ...
                $.ajax({
                    type: "POST",
                    url: "{{ url('') }}/api/biaya_bahan_penolong/create/"+id,
                    dataType: "json",
                    data: datas
                }).done(function(data) {
                    if (data.error == false) {
                    ...
                    $('#add').modal('hide')
                    document.getElementById("forminput").reset();
                    $("#refreshselect").load(location.href + " #bahan_id");
                    $("#refreshselect2").load(location.href + " #editbahan_id");
                })
            });
            ...
            $('#formupdate').on('submit', function(e) {
                ...
                $.ajax({
                    type: "POST",
                    url: "{{ url('') }}/api/biaya_bahan_penolong/update/"+id,
                    dataType: "json",
                    data: datas
                }).done(function(data) {
                    if (data.error == false) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        type: "DELETE",
                        dataType: "json",
                        url: "{{ url('') }}/api/biaya_bahan_penolong/delete/" + value
                    }).done(function(data) {
                        if (data.error == false) {
                            Swal.fire({
                                icon: 'success',
