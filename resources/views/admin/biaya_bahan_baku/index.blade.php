            ...
            <div class="row">
                <div class="col-lg-3">
                    <div class="list-group">
                        <a href="{{Session::get('base_url')}}/biaya_bahan_baku/{{$produksi_id}}" class="list-group-item list-group-item-action active" aria-current="true">
                          Biaya Bahan Baku
                        </a>
                                                    ...
                                                    <div class="mb-3">
                                                        ...
                                                        <div id="refreshselect">
                                                            <select class="form-select" id="bahan_id" name="bahan_id"
                                                                ...
                                                                @inject('BiayaBahanBaku', 'App\Models\BiayaBahanBaku')
                                                                @foreach ($bahan_baku as $item)
                                                                @php
                                                                    $biaya_bahan_baku = $BiayaBahanBaku::where('bahan_id',$item->id)->where('produksi_id',$produksi_id)->first();
                                                                @endphp
                                                                    <option value="{{ $item->id }}" {{!is_null($biaya_bahan_baku) ? 'disabled' : ''}}>{{ $item->nama_bahan }} </option>
                                                                @endforeach
                                                            </select>
                                                        </div>
                                                    </div>
...
@push('script')
    ...
    <script>
         function loadtable() {
            ...
            setTimeout(() => {
                $('#table').load('../table_biaya_bahan_baku/'+id, function() {
                    $('#load_table').hide();
                });
            }, 500);
        }
        ...
        function editdata(value) {
            ...
            $('#editqty').val('');
            setTimeout(() => {
                $.get('{{ url('') }}/api/biaya_bahan_baku/show/' + value, function(data,
                    status) {
                    if (data.error == false) {
                        $('#editid').val(value);
        ...
        $(document).ready(function() {
            ...
            $('#forminput').on('submit', function(e) {
                ...
                $.ajax({
                    type: "POST",
                    url: "{{ url('') }}/api/biaya_bahan_baku/create/"+id,
                    dataType: "json",
                    data: datas
                }).done(function(data) {
                    if (data.error == false) {
            ...
            $('#formupdate').on('submit', function(e) {
                ...
                $.ajax({
                    type: "POST",
                    url: "{{ url('') }}/api/biaya_bahan_baku/update/"+id,
                    dataType: "json",
                    data: datas
                }).done(function(data) {
                    if (data.error == false) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        type: "DELETE",
                        dataType: "json",
                        url: "{{ url('') }}/api/biaya_bahan_baku/delete/" + value
                    }).done(function(data) {
                        if (data.error == false) {
                            Swal.fire({
                                icon: 'success',
