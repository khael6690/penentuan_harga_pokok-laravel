                                            ...
                                            <div class="modal-body">
                                                <form id="forminput">
                                                    ...
                                                    <div class="mb-3">
                                                        ...
                                                        <textarea class="form-control" id="alamat" name="alamat" cols="30" rows="10" required></textarea>
                                                    </div>
                                        ...
                                        <div class="modal-content">
                                            ...
                                            <div class="modal-body">
                                                <form id="formupdate">
                                                    ...
                                                    <div class="mb-3">
                                                        <label class="form-label">Alamat</label>
                                                        <textarea class="form-control" id="editalamat" name="alamat" cols="30" rows="10" required></textarea>
                                                    </div>
...
@push('script')
    ...
    <script>
        function loadtable() {
            $('#load_table').show();
            setTimeout(() => {
                $('#table').load('./table_pelanggan', function() {
                    $('#load_table').hide();
                });
            }, 500);
        }
        ...
        function editdata(value) {
            $('#editnama_pelanggan').val('');
            $('#editalamat').val('');
            setTimeout(() => {
                $.get('{{ url('') }}/api/pelanggan/show/' + value, function(data,
                    status) {
                    if (data.error == false) {
                        $('#editid').val(value);
                        $('#editnama_pelanggan').val(data.result.nama_pelanggan);
                        $('#editalamat').val(data.result.alamat);
                    }
                });
            }, 100);
        }
        ...
        $(document).ready(function() {
            ...
            $('#formupdate').on('submit', function(e) {
                ...
                $.ajax({
                    type: "POST",
                    url: "{{ url('') }}/api/pelanggan/update/"+id,
                    dataType: "json",
                    data: datas
                }).done(function(data) {
                    if (data.error == false) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        type: "DELETE",
                        dataType: "json",
                        url: "{{ url('') }}/api/pelanggan/delete/" + value
                    }).done(function(data) {
                        if (data.error == false) {
                            Swal.fire({
                                icon: 'success',
